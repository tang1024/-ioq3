 ...
 * creating an entity and defining it as a rocket (also called a missile).
 * @param[in] self The parent entity (owner).
 * @param[in] start A starting location.
 * @param[in] dir A direction to face.
...
gentity_t* fire_rocket(gentity_t* self, vec3_t start, vec3_t dir)
{
    ...
    bolt = G_Spawn();                        // Either finds a free entity or creates one.
    bolt->classname = "rocket";
    bolt->nextthink = level.time + 15000;    // Set the time until the next think period.
    bolt->think = G_ExplodeMissile;          // Specifies which think function will be called.
    bolt->s.eType = ET_MISSILE;              // Type of bolt.
    bolt->r.svFlags = SVF_USE_CURRENT_ORIGIN;// Link position.
    bolt->s.weapon = WP_ROCKET_LAUNCHER;     // The weapon it was fired from.
    bolt->r.ownerNum = self->s.number;       // Its owner.
    bolt->parent = self;

    bolt->damage = 100;
    bolt->splashDamage = 100;                       // Indirect damage.
    bolt->splashRadius = 120;
    ...
    bolt->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME; // move a bit on the very first frame
    VectorCopy(start, bolt->s.pos.trBase);
    VectorScale(dir, 900, bolt->s.pos.trDelta);           // Fired at 900 units/sec.
    SnapVector(bolt->s.pos.trDelta);                      // save net bandwidth
    VectorCopy(start, bolt->r.currentOrigin);
