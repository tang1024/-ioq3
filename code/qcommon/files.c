...
/**
 * File system (handle based) for Quake III Arena.
 * @file
 *
...
// than the demo pak, even if product id is present.  This is only used for our
...
char lastValidBase[MAX_OSPATH];         ///< Last valid game folder used.
char lastValidComBaseGame[MAX_OSPATH];  ///< Last valid game folder used.
char lastValidFsBaseGame[MAX_OSPATH];   ///< Last valid game folder used.
char lastValidGame[MAX_OSPATH];         ///< Last valid game folder used.
 ...
 * it will return the size of the pak file, not the expected size of the file.
 *
 * @param f
 * @return
 */
 ...
 * This is to determine if opening a file to write (which always goes into the current gamedir)
 * will cause any overwrites.
 *
 ...
 * @param uniqueFILE
 * @param unpure
 * @return filesize and an open FILE pointer.
 */
 ...
 *
 * If uniqueFILE is true, then a new FILE will be fopened,
 * even if the file is found in an already open pak file.
 * If uniqueFILE is false, you must call FS_FCloseFile instead of fclose, 
 * otherwise the pak FILE would be improperly closed.
 ...
 * @param file
 * @param uniqueFILE
 * @return Returns filesize and an open FILE pointer.
 */
 ...
 * @param name
 * @param enableDll
 * @return Return the searchpath in "startSearch".
 */
int FS_FindVM(void** startSearch, char* found, int foundlen, const char* name, int enableDll)
{
 ...
 /**
  * Properly handles partial reads. Properly handles partial reads and reads from other dlls.
  *
  * @param buffer
  * @param len
  * @param f
 ...
 * @param unpure
 * @param buffer
 * @return The length of the file. A null buffer will just return the file length without loading.
 */
 ...
 * @param listbuf
 * @param bufsize
 * @return Returns a list of mod directory names.
 */
...
#ifndef STANDALONE
/**
 * Check whether any of the original id pak files is present, and start up in standalone mode,
 * if there are none and a different com_basegame was set.
 *
 * Note: If you're building a game that doesn't depend on the Q3 media pak0.pk3,
 * you'll want to remove this by defining
 ...
 * If the string is empty, all data sources will be allowed.
 *
 * If not empty, only pk3 files that match one of the space separated checksums will be checked for files,
 * with the exception of .cfg and .dat files.
 *
 * @param pakSums
 * @param pakNames
 */
 ...
 * @param checksumFeed
 * @param disconnect
 * @return Return qtrue if restarting due to game directory changed, qfalse otherwise.
 */
