...
/// Used for system variables, not for player specific configurations.
 ...
 * CVARS (console variables).
 * 
 * Many variables can be used for cheating purposes, 
 * so when cheats is zero, force all unspecified variables to their default values.
 */
struct cvar_s {
    char*       name;
    char*       string;
 ...
 * so if a playerState_t is transmitted, the entityState_t can be fully derived from it.
 */
 ...
 *
 * The messages are delta compressed,
 * so it doesn't really matter if the structure size is fairly large.
 */
